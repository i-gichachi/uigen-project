...
function getToolLabel(toolName: string, args: Record<string, any>): string {
  ...
  if (toolName === "str_replace_editor") {
    switch (args?.command) {
      ...
      default:         return fileName ? `Updating ${fileName}` : "Updating file";
    }
  }
  ...
  if (toolName === "file_manager") {
    switch (args?.command) {
      case "rename": {
        const from = args.path?.split("/").pop();
        const to = args.new_path?.split("/").pop();
        return from && to ? `Renaming ${from} → ${to}` : "Renaming file";
      }
